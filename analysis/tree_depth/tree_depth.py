...
GOLD_DEPTH = []
for line in open("small_gold_parses_tree_depth.txt", "r"):
    line = line.strip()
    max_depth = 0
    n_open = 0
    n_closed = 0
...
for i in range(5):
    ...
    sent_num = -1
    sentences = {}
    for line in open(f"bllip_particle/bllip_tree_depth-{i+1}.txt", "r"):
        line = line.split("\t")
    ...
    DIFFERENCES = [GOLD_DEPTH[i] - TEST_DEPTH[i] for i in range(len(GOLD_DEPTH))]
    DEPTH_DIFFERENCES_TO_AVERAGE.append(DIFFERENCES)

AVERAGES = [np.mean(np.asarray([currlist[i] for currlist in DEPTH_DIFFERENCES_TO_AVERAGE])) for i in range(len(GOLD_DEPTH))]

#calculate the differences between the gold and test depth
print(f"On average, gold parses are {np.mean(AVERAGES)} deeper than predicted parses (stderr = {np.std(AVERAGES)/np.sqrt(len(AVERAGES))})")
