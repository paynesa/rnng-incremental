    ...
    while i <= max_idx:
        ...
        if line_strip[i] == '(':
            if is_next_open_bracket(line_strip, i): # open non-terminal
                ...
                curr_NT = 'NT(' + curr_NT + ')'
                # store the NTs since they will be proposed after the first shift after they are opened 
                unused_nts.append(curr_NT)
                i += 1  
                while line_strip[i] != '(': # get the next open bracket, which may be a terminal or another non-terminal
                    i += 1
            else: # it's a terminal symbol
                output_actions.append('SHIFT')
                # when there are open NT's right before the word, then we shift the word and then propose the NTs
                for unused_nt in unused_nts:
                    output_actions.append(unused_nt)
                unused_nts = []
                while line_strip[i] != ')':
                    i += 1
                i += 1
...
def main():
    ...
    # the first command-line argument is the training file -- read those lines and get the vocabulary from it 
    train_file = open(sys.argv[1], 'r')
    lines = train_file.readlines()
    train_file.close()
    words_list = get_dictionary.get_dict(lines) 
    # the second command line argument is the dev file -- read those lines 
    dev_file = open(sys.argv[2], 'r')
    dev_lines = dev_file.readlines()
    dev_file.close()
    line_ctr = 0
    # get the oracle for the dev file
    for line in dev_lines:
        ...
        # first line: the bracketed tree itself 
        print '# ' + line.rstrip()
        tags, tokens, lowercase = get_tags_tokens_lowercase(line)
        assert len(tags) == len(tokens)
        assert len(tokens) == len(lowercase)
        # second line: the terminals of the tree 
        print ' '.join(tokens)
        unkified = unkify(tokens, words_list)    
        # third line: the UNK-ified terminals 
        print ' '.join(unkified)
        # fourth line: the action sequence
